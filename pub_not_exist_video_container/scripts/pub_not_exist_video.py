...
@click.option(
    ...
    type=str,
    required=True,
    help="not exist videoid list"
)             
def main(schema, host, port, login, password, vhost, routing_key, not_exist_video_id_list):
    tx, chan = get_tx_and_chan(schema, host, port, login, password, vhost)
    if isinstance(not_exist_video_id_list, str):
        not_exist_video_id_list = eval(not_exist_video_id_list)
    pub_video_id_list(tx, chan, routing_key, not_exist_video_id_list[:3]) #for test
